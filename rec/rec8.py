...
def simulate_init(query):
    ...
    while day<T:
        ...
        # できそうなアクションの列挙
        # 序盤はmachineふやす
        if day<=820 and len(pos_machine)<55 and money>=(len(pos_machine)+1)**3: # 増やせる->一番大きいのを探す
            max_val=-1
            max_r=0
            max_c=0
            for i in range(N):
        ...
        if len(pos_machine)>=5 and calc_cnt2(pos_machine,pr,pc)!=len(pos_machine):
            min_cnt=1000
            max_cnt=-1
            min_r,min_c=-1,-1
            max_r,max_c=-1,-1
        ...
        if best_r>=0 and best_v*len(pos_machine)/2.5>best_sc: # この枝刈りがめちゃくちゃ効く
            ...
            # not_splitの状態で展開している以上calc_cntは固定では？
