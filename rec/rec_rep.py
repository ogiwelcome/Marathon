    ...
    for pr,pc in pos:
        for nr,nc in [[pr-1,pc],[pr+1,pc],[pr,pc-1],[pr,pc+1]]:
            if 0<=nr<N and 0<=nc<N:
                candi.add((nr,nc))
    return candi
def calc_candi_depth(pos,depth):
    q=[]
    candi=set()
    for mr,mc in pos:
        q.append([mr,mc,0])
    ...
    for r,c in candi:
        ...
        min_d=10000
        for mr,mc in pos:
            if min_d>abs(mr-r)+abs(mc-c):
                min_d=abs(mr-r)+abs(mc-c)
                min_mr=mr
                min_mc=mc
            if min_d<depth:
                break
        if min_d==depth:
            new_candi.add((min_mr,min_mc,r,c))
    return new_candi
#kernprof -l -v rec11.py
#@profile
def calc_candi_depth(pos,depth):
    q=[]
    candi=set()
...
def simulate_init(query):
    money=1
    pos_machine=set()
    pos_vege=[[[] for i in range(N)] for j in range(N)]
    for R,C,S,E,V in query:
        pos_vege[R][C].append((S,E,V))
    for i in range(N):
        for j in range(N):
            pos_vege[i][j].sort(key=lambda x:x[1])
    act=[]
    ...
    while day<T:
        ...
        # できそうなアクションの列挙
        # 序盤はmachineふやす
        pos_machine_l=list(pos_machine)
        if day<=800 and money>=(len(pos_machine)+1)**3: # 増やせる->一番大きいのを探す
            max_val=-1
            max_r=0
            max_c=0
            for i in range(N):
                for j in range(N):
                    if (i,j) in pos_machine_l:continue
            ...
            if max_val>=0:
                ...
                for i in range(N):
                    for j in range(N):
                        if (not did) and ((i,j) not in pos_machine_l):
                            did=True
                            money-=(len(pos_machine)+1)**3
                            pos_machine.add((i,j))
                            act.append([i,j])
        ...
        pr,pc=pos_machine_l[0]
        if calc_cnt2(pos_machine,pr,pc)!=len(pos_machine):
            min_cnt=1000
            max_cnt=-1
            min_r,min_c=-1,-1
            max_r,max_c=-1,-1
            for mr,mc in pos_machine_l:
            ...
            while q:
                ...
                for ny,nx in [[y-1,x],[y+1,x],[y,x-1],[y,x+1]]:
                    ...
                    if (ny,nx) not in vis:
                        vis.add((ny,nx))
                    if (ny,nx) not in vis and (ny,nx) in pos_machine_l: # changed
                        vis.add((ny,nx))
                        q.append((ny,nx))
            vis-=pos_machine
            best_ac=[]
                ...
                for s,e,v in pos_vege[r][c]:
                    ...
                    if s<=day<=e:
                        ...
                        if sc>best_sc:
                            ...
                            best_ac=[pr,pc,r,c]
                            best_l=1
                            max_len=len(calc_candi(pos_machine2))
                        # add
                        elif sc==best_sc:
        ...
        best_mr=-1
        best_mc=-1
        candi_d2=calc_candi_depth(pos_machine,2)
        for mr,mc,r,c in candi_d2:
            # この時点で二手
            for s,e,v in pos_vege[r][c]:
                if s<=day+1<=e:
                    if v>best_v:
        ...
        best_mr=-1
        best_mc=-1
        candi_d3=calc_candi_depth(pos_machine,3)
        for mr,mc,r,c in candi_d3:
            # この時点で二手
            for s,e,v in pos_vege[r][c]:
                if s<=day+2<=e:
                    if v>best_v:
...
for rep in range(100):
    N,M,T,query=generate()
    money,act=simulate_init(query)
    ss+=money
    print(rep,ss/(rep+1))
ss/=100
print("ans:",ss)