...
def solve(N,tasks):
    ...
    route=tasks[:50]
    route1=[[400,400,-1]]+[[route[i][0],route[i][1],route[i][4]] for i in range(50)]
    route2=[[route[i][2],route[i][3],route[i][4]] for i in range(50)]+[[400,400,-1]]
    min_dist=0
    ...
    while True:
        loop+=1
        if loop%100==0 and time.time()-time0>1:
            break
        # まず隣接swap
        i1=random.randint(1,48)
        i2=random.randint(1,48)
        while i1==i2:
    ...
    while True:
        loop+=1
        if loop%100==0 and time.time()-time0>1.7:
            break
        # まず隣接swap
        i1=random.randint(1,48)
        i2=random.randint(1,48)
    ...
    for i in range(len(route1)-1):
        a1,b1,idx1=route1[i]
        a2,b2,idx2=route1[i+1]
        have_set.add(idx1)
        order+=[a1,b1]
        split=-1
        for j in range(1,len(route2)-1):
            c3,d3,idx3=route2[j-1]
            c4,d4,idx4=route2[j]
            c5,d5,idx5=route2[j+1]
            if not idx4 in have_set:
                continue
            delta=0
            delta-=dist(a1,b1,a2,b2)+dist(c3,d3,c4,d4)+dist(c4,d4,c5,d5)
            delta+=dist(a1,b1,c4,d4)+dist(c4,d4,a2,b2)+dist(c3,d3,c5,d5)
            if delta<0:
                min_dist+=delta
                order+=[c4,d4]
                break
        if split>=0:
            route2=route2[:split]+route2[split+1:]
    order+=route1[-1][:2]
    if DEBUG:
        print(loop)
