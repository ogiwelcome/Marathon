...
def calc_score_changed_type1(arr,score_arr,days_arr,day_num,after):
    ...
    if delta_before+delta_after>score_arr[before]+score_arr[after]:
        n_score[before]=delta_before
        n_score[after]=delta_after
        return n_score,n_days,n_arr
    else:
        return score_arr,days_arr,arr
def calc_score_changed_type2(arr,score_arr,days_arr,day_num1,day_num2):
    n_score=[score_arr[i] for i in range(len(score_arr))]
    n_days=[[days[i][j] for j in range(len(days[i]))] for i in range(26)]
    n_arr=[arr[i] for i in range(len(arr))]
    ...
    if delta_before1+delta_before2>score_arr[before1]+score_arr[before2]:
...
cnt_loop=0
while time.time()-time0<17:
    cnt_loop+=1
    flg+=1;flg%=2
    if flg:
        num=random.randint(0,D-1)
        after=random.randint(0,25)
        score,days,out=calc_score_changed_type1(out,score,days,num,after)
    else:
        num1=random.randint(0,D-1)
        num2=random.randint(0,D-1)
        while num1==num2:
            num2=random.randint(0,D-1)
        score,days,out=calc_score_changed_type2(out,score,days,num1,num2)
print(cnt_loop)
print(calc_score(out))
